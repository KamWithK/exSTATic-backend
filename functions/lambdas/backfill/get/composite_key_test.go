...
func TestValidDates(t *testing.T) {
	...
	key, date, err = user_media.SplitUserMediaCompositeKey("type#username", user_media.ZeroPadInt64(0)+"#identifier")
	assert.NotEmpty(t, key)
	assert.EqualValues(t, *date, 0)
	assert.NoError(t, err)

	key, date, err = user_media.SplitUserMediaCompositeKey("type#username", user_media.ZeroPadInt64(101)+"#identifier")
	assert.NotEmpty(t, key)
